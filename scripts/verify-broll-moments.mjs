...
// through the panel's real file input with a 0:02-0:05 range. It verifies by
// sampling the center canvas region that during playback and while scrubbing
// the magenta image renders ONLY inside [2s,5s) — absent at 1s (plain
// speaker colors), present at 3.5s, absent again at 6s — then repeats the
...
// back into a <video>, seeks BEFORE (1s), DURING (3.5s), and AFTER (6s) the
// moment, draws each decoded frame to a probe canvas, and asserts the
// magenta overlay is burned in ONLY at 3.5s with real nonblank speaker
// frames at 1s and 6s. All pixel assertions are region-based and tolerant of
...
(async () => {
  ...
  // ~8.2s solid-color speaker video (uniform frames so the b-roll region is
  ...
  // Model semantics: b-roll validation requires an image; [start, end) holds.
  {
    ...
    assert(/image/i.test(M.validateMoment({ type: "broll", start: 2, end: 5 })), "b-roll without an image must fail validation");
    assert(M.validateMoment({ type: "broll", imageName: "broll.png", start: "0:02", end: "0:05" }) === "", "b-roll with an image and valid range must validate");
    const b = M.addMoment(scratch, { type: "broll", imageName: "broll.png", start: 2, end: 5 });
    ...
    assert(at(1) === "" && at(2) === "broll" && at(3.5) === "broll" && at(5) === "" && at(6) === "", "b-roll must follow [start,end) activation");
  ...
  assert(listText.includes("0:02") && listText.includes("0:05"), "list should show the b-roll range: " + listText);
  ...
  await waitFor(() => brollAbsent(), "b-roll should not render at the start of playback (before 2s)", 30);
  await waitFor(() => brollShown(), "b-roll image should appear during playback inside 2-5s (Split)", 120);
  await waitFor(() => brollAbsent(), "b-roll image should disappear once playback passes 5s", 200);
  ...
  pausePreview();
  await scrubTo(1);
  await waitFor(() => brollAbsent(), "scrubbed to 1s: b-roll absent, plain speaker colors (Split)");
  const splitBefore = regionStats(stage(), BROLL_REGION);
  await scrubTo(3.5);
  await waitFor(() => brollShown(), "scrubbed to 3.5s: b-roll image shown (Split)");
  const splitDuring = regionStats(stage(), BROLL_REGION);
  await scrubTo(6);
  await waitFor(() => brollAbsent(), "scrubbed to 6s: b-roll absent again (Split)");
  ...
  assert(pausedDuring.magenta > 0.5, "PAUSED scrub to 3.5s must draw the overlay on the paused canvas: " + JSON.stringify(pausedDuring));
  await sleep(400);
  const pausedStill = regionStats(stage(), BROLL_REGION);
  assert(pausedStill.magenta > 0.5, "overlay must PERSIST on the paused canvas at 3.5s: " + JSON.stringify(pausedStill));
  scrubViaChange(6);
  await sleep(700);
  const pausedAfter = regionStats(stage(), BROLL_REGION);
  assert(pausedAfter.magenta < 0.05 && pausedAfter.bright > 0.4, "PAUSED scrub to 6s must clear the overlay while keeping real speaker frames: " + JSON.stringify(pausedAfter));
  ...
  pausePreview();
  await scrubTo(3.5);
  await waitFor(() => brollShown(), "Stack at 3.5s: b-roll image should render over the recomposed layout");
  const stackDuring = regionStats(stage(), BROLL_REGION);
  await scrubTo(6);
  await waitFor(() => brollAbsent(), "Stack at 6s: b-roll absent outside its range");
  ...
  assert(v.duration >= 6.5, "export should cover before/during/after the b-roll range, duration=" + v.duration);
  ...
  assert(before.broll.magenta < 0.05 && before.broll.bright > 0.4, "no b-roll may be burned in at 1s: " + JSON.stringify(before.broll));
  assert(during.frame.bright > 0.2, "exported frame at 3.5s should be nonblank");
  assert(during.broll.magenta > 0.4, "b-roll image should be burned into the exported frame at 3.5s: " + JSON.stringify(during.broll));
  assert(after.frame.bright > 0.2, "exported frame at 6s should be nonblank");
  assert(after.broll.magenta < 0.05 && after.broll.bright > 0.4, "no b-roll may be burned in at 6s: " + JSON.stringify(after.broll));
...
async function main() {
  ...
  try {
    ...
    // 120s budget: two ~8s in-browser media generations plus a generated PNG,
    ...
    console.log("verify-broll-moments: OK — timed b-roll PNG renders only inside 2-5s across Split/Stack (playing AND paused-scrub) and is burned into the export (absent at 1s/6s, present at 3.5s)");
