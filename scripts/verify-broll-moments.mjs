...
(async () => {
  ...
  await waitFor(() => brollShown(), "b-roll image should appear during playback inside 2-6s (Split)", 120);
  await waitFor(() => brollAbsent(), "b-roll image should disappear once playback passes 6s", 200);
  ...
  await scrubTo(7);
  await waitFor(() => brollAbsent(), "scrubbed to 7s: b-roll absent again (Split)");
  ...
  assert(pausedDuring.magenta > 0.5, "PAUSED scrub to 4s must draw the overlay on the paused canvas: " + JSON.stringify(pausedDuring));
  await sleep(400);
  const pausedStill = regionStats(stage(), BROLL_REGION);
  assert(pausedStill.magenta > 0.5, "overlay must PERSIST on the paused canvas at 4s: " + JSON.stringify(pausedStill));
  scrubViaChange(7);
  await sleep(700);
  const pausedAfter = regionStats(stage(), BROLL_REGION);
  assert(pausedAfter.magenta < 0.05 && pausedAfter.bright > 0.4, "PAUSED scrub to 7s must clear the overlay while keeping real speaker frames: " + JSON.stringify(pausedAfter));
  ...
  pausePreview();
  await scrubTo(4);
  await waitFor(() => brollShown(), "Stack at 4s: b-roll image should render over the recomposed layout");
  const stackDuring = regionStats(stage(), BROLL_REGION);
  await scrubTo(7);
  await waitFor(() => brollAbsent(), "Stack at 7s: b-roll absent outside its range");
  ...
  assert(v.duration >= 7.5, "export should cover before/during/after the b-roll range, duration=" + v.duration);
  ...
  assert(during.frame.bright > 0.2, "exported frame at 4s should be nonblank");
  assert(during.broll.magenta > 0.4, "b-roll image should be burned into the exported frame at 4s: " + JSON.stringify(during.broll));
  assert(after.frame.bright > 0.2, "exported frame at 7s should be nonblank");
  assert(after.broll.magenta < 0.05 && after.broll.bright > 0.4, "no b-roll may be burned in at 7s: " + JSON.stringify(after.broll));
