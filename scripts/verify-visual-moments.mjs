...
(async () => {
  ...
  await waitFor(() => titleShown() && calloutAbsent(), "title should appear during playback inside 0-4s (Split)", 120);
  await waitFor(() => titleAbsent(), "title should disappear once playback passes 0:04", 200);
  await waitFor(() => calloutShown() && titleAbsent(), "callout should appear during playback inside 5-8s (Split)", 200);
  ...
  await scrubTo(4.5);
  await waitFor(() => titleAbsent() && calloutAbsent(), "scrubbed to 4.5s: neither moment shown (Split)");
  assert(regionStats(stage(), TITLE_REGION).bright > 0.5, "at 4.5s the title region should show plain bright video");
  await scrubTo(6);
  await waitFor(() => calloutShown() && titleAbsent(), "scrubbed to 6s: callout shown, title absent (Split)");
  ...
  for (const presetId of ["stack", "spotlight"]) {
    ...
    await waitFor(() => titleShown(), presetId + ": title should render over the recomposed layout inside 0-4s", 120);
    pausePreview();
    await scrubTo(4.5);
    await waitFor(() => titleAbsent() && calloutAbsent(), presetId + ": neither moment at 4.5s");
    await scrubTo(6);
    await waitFor(() => calloutShown() && titleAbsent(), presetId + ": callout shown (and title absent) at 6s");
  ...
  assert(v.duration >= 7, "export should cover both moment ranges, duration=" + v.duration);
  ...
  assert(inTitle.frame.bright > 0.2, "exported frame at 2s should be nonblank");
  ...
  assert(plainVideo(inGap.title) && inGap.title.light < 0.02, "no title should be burned in at 4.5s: " + JSON.stringify(inGap.title));
  assert(plainVideo(inGap.callout), "no callout should be burned in at 4.5s: " + JSON.stringify(inGap.callout));
  assert(inCallout.frame.bright > 0.2, "exported frame at 6s should be nonblank");
  assert(burnedIn(inCallout.callout), "callout overlay should be burned into the exported frame at 6s: " + JSON.stringify(inCallout.callout));
  assert(plainVideo(inCallout.title), "no title should be burned in at 6s: " + JSON.stringify(inCallout.title));
