...
test("media loading, seeking, playback, and recorder stop have timeouts", () => {
  assert.match(source, /waitForVideoReady\(entry\.video, 10000\)/);
...
test("valid setup auto-composes preview from uploaded media", () => {
  assert.match(source, /scheduleAutoPreview/);
  assert.match(source, /assignFileToBucket/);
