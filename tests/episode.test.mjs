...
test("custom layouts can be saved, reapplied, and survive preset round-trips", () => {
  ...
  assert.ok(template, "template should save");
  assert.equal(ep.activeTemplateId, template.id);
  assert.equal(ep.activeLayoutMode, "template");
  assert.equal(E.listTemplates(ep).length, 1);
  assert.deepEqual(E.getTemplate(ep, template.id).rects.host, draft.host);

  E.setPreset(ep, "stack");
  assert.equal(ep.activeTemplateId, null, "preset switch should clear the active template");
  assert.equal(E.getActiveLayout(ep).kind, "preset");

  assert.equal(E.applyTemplate(ep, template.id), true, "saved template can be applied again");
