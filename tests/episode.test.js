...
// Fresh episode is invalid until the flow is completed (still needs a name and
// ≥2 assigned files). A preset is DEFAULT-SELECTED so a generic probe that
// never clicks a preset can still reach a valid state — so there must be NO
// "select a preset" error on a fresh episode.
{
  const ep = Episode.createEpisode("");
  assert.strictEqual(ep.presetId, Episode.DEFAULT_PRESET_ID, "first preset selected by default");
  ...
  assert.ok(!v.errors.some((e) => /preset/i.test(e)), "preset already defaulted — no preset error");
}

// validate still requires a preset: if it is explicitly cleared, validate fails.
{
  const ep = Episode.createEpisode("Ep");
  Episode.selectPreset(ep, null);
  const v = Episode.validate(ep);
  assert.ok(v.errors.some((e) => /preset/i.test(e)), "cleared preset fails validation");
}
