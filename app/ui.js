...
(function () {
  ...
  // Start decoding the PNG the instant it's chosen — not when "Add moment" is
  // later clicked — so decoding has almost always finished well before the
  // moment exists, instead of racing the very first frame that should show it.
  // pendingImage.assign(moment.id) (below, in the add handler) (re-)targets
  // the in-flight (or already-finished) decode at the real moment id.
  ...
  momentImageInput.addEventListener("change", function () {
    ...
    if (file) {
      ...
      pendingImage = PDC.momentImages.preload(file);
    } else {
      pendingImage = null;
    }
  ...
  $("moment-add").addEventListener("click", function () {
    ...
    if (type === "image" && imageFile && pendingImage) {
      pendingImage.assign(moment.id);
    }
    pendingImage = null;
    showMomentError("");
    $("moment-text").value = "";
