(function initializePodcastDesignCanvasModel() {
  ...
  function validateSetup(setup) {
    ...
    for (const upload of assigned) {
      const social = setup.socials[upload.bucket] || "";
      if (!social.trim()) {
        errors.push(`Add a social link for ${getBucketLabel(upload.bucket)}.`);
      } else if (!looksLikeUrl(social)) {
        errors.push(`${getBucketLabel(upload.bucket)} needs a valid social URL.`);
      }
    }
  ...
  function formatSocialLabel(value) {
    if (!value) {
      return "";
    }
    try {
      ...
      return handle ? `@${handle.replace(/^@/, "")}` : url.hostname.replace(/^www\./, "");
    } catch {
