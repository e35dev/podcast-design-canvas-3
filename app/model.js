...
function buildAssignmentMap(uploads) {
...
function validateSetup({ uploads, socials, presetId }) {
  const errors = [];
...
function getEpisodeDuration(mediaEntries) {
...
function getSpotlightIndex(order, elapsedMs, cycleMs) {
  if (!order.length) {
    return 0;
  }
...
function getFrames(presetId, assignedBuckets, width, height, elapsedMs = 0) {
