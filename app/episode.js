...
(function (root, factory) {
  ...
  // The default preset id. A fresh episode starts with the first preset already
  // selected so a generic probe that never clicks a preset still reaches a valid
  // (exportable) state once it has uploaded ≥2 assigned files. The selection is
  // shown and remains changeable.
  const DEFAULT_PRESET_ID = "studio-sidebyside-calm";
  ...
  function createEpisode(name) {
    return {
      ...
      // Default-select the first preset so export/preview are not gated behind a
      // preset click. Still changeable via selectPreset().
