(function initializePodcastDesignCanvasApp() {
  ...
  document.querySelectorAll(".speaker-slot").forEach((slot) => {
    ...
    slot.addEventListener("drop", (event) => {
      ...
      if (file) {
        assignFileToBucket(bucket, file);
      }
    });
  });
  ...
  function assignFileToBucket(bucket, file) {
    ...
    renderReadyList();
    renderNativePreview();
    const input = document.querySelector(`[data-file-for="${bucket}"]`);
    if (input && file) {
      const transfer = new DataTransfer();
      transfer.items.add(file);
      input.files = transfer.files;
    }
    drawEmptyCanvas(file ? "Composing preview from uploaded media..." : "Upload Host and Guest 1 videos, add social links, then preview.");
    if (file) {
      setStatus(`${getBucketLabel(bucket)} video assigned: ${file.name}`);
    }
    scheduleAutoPreview();
  }
  ...
  function scheduleAutoPreview() {
    ...
    state.autoPreviewTimer = setTimeout(() => {
      ...
      if (!errors.length && !state.prepared.length && !state.exporting && !state.previewing) {
        handlePreview();
      }
    }, 400);
  }
  ...
  async function handlePreview() {
    if (state.previewing || state.exporting) {
      return;
    }
    ...
    const ready = await Promise.all(state.prepared.map((entry) => waitForVideoReady(entry.video, 10000)));
